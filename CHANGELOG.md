...
- Windows 字体固定使用已注册的 Microsoft YaHei UI Regular/Bold，字重收敛到 400/700；旧窗口几何限制到屏幕可用区。收件页改用高内容页和常驻页面滚动条，恢复舒展留白并为今日文件、最近日志各保留 220 像素以上高度；文件表格上的滚轮直接驱动页面滚动，避免日志区域无法到达。左右侧栏适度收窄以加宽中间工作区，文件表格无需横向滚动即可操作。右侧面板保留独立滚动，并完成 100%、125%、150% 与深色主题截图 QA。
