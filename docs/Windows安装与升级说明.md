...
覆盖升级保留 Agent Client、权限、配置备份、邮件、附件、OAuth、Credential 和审计。普通卸载继续保留 `%LOCALAPPDATA%\AgentMailBridge` 与 Credential Manager 数据，重装后恢复读取；外部 Codex/Claude 配置不会由卸载器静默删除，用户可在升级前后从“Agent 接入”暂停、撤销或恢复自身备份。
