...
每个 Agent Client 另有独立随机 scoped token。管理副本位于 Windows Credential Manager，SQLite 只保存 SHA-256 与不含秘密的 credential reference。它只能代表该 Client 在 AgentMailBridge 内获准的 capability、账号和工作区，不能推导或读取邮箱凭据。
...
暂停 Client 会保留 token 但立即拒绝调用；轮换会生成新 token 并使旧值失效；撤销会删除 Credential Manager 管理副本并立即拒绝旧配置。撤销外部配置时只移除 `agent-mail-bridge` 项，其他 MCP Server 不受影响。
