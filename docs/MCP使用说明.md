...
AgentMailBridge v1.4.0 MCP 是本机按需启动的统一 stdio 服务。它不按 Provider 拆分，不监听端口、不常驻、不创建快捷方式或托盘；兼容 MCP 的本机 Agent 启动进程，stdin 关闭后服务退出。七工具、`submit_result` 参数、只读 Gmail scope 和固定 owner 安全边界保持兼容。不同客户端和邮箱账号使用同一个服务、配置和工具，不存在客户端或 Provider 专用业务端点。
...
- `get_mail_sync_status`：返回当前已接通收件账号的自动收件、上次检查/成功、下次检查、重试、新鲜度、跨进程同步状态及已启用账号摘要。
...
v1.4.0 的 `account_id` 是稳定的不透明标识。MCP 未指定时保持统一视图，指定时严格限制 Mail Facts 查询；当前 `ensure_fresh` 仍只同步 GUI 中已接通的 Gmail 收件账号，不会假装同步尚未实现的 Provider。
