...
- `search_mails`：按 latest、today、yesterday、recent_days、date_range 或 all 搜索，支持 query、主题、发件人、收件人、有无附件、状态、排序、分页及 `ensure_fresh`。
- `get_mail`：按稳定 mail_id/package_id 返回元数据、有界正文、资源清单和真实 raw.eml 描述；长正文用 offset/max_chars 分页。
...
- `get_mail_sync_status`：返回自动收件、上次检查/成功、下次检查、重试、新鲜度和跨进程同步状态。
...
`ensure_fresh=true` 只在本地数据过期时触发受控同步。GUI、自动调度和 Agent 共用操作系统级跨进程锁；其他进程正在收件时返回 `sync_in_progress`。同步失败且允许缓存时，搜索明确返回 cached 与 sync_error；不得把旧数据伪装为新鲜。
