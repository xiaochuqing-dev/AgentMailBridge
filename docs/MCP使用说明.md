...
AgentMailBridge v1.6.0 MCP 是本机按需启动的统一 stdio 服务。它不按 Provider 或 Client 拆分，不监听端口、不常驻、不创建快捷方式或托盘；stdin 关闭后服务退出。七工具、`submit_result` 参数、只读 Gmail scope 和固定 owner 安全边界保持兼容。Codex、Claude Code 与 Hermes 查询同一份 Gmail、QQ、163 和 Generic 本地邮件事实。
...
- `prepare_mail_resources`：兼容模式把指定资源复制到允许目录；`mode=complete` 无需 resource_ids，原子准备正文、真实 raw.eml、邮件信息、附件、邮件内图片、已归档下载和来源 manifest。两种模式都校验 ownership、路径、大小与 SHA-256。
...
v1.6.0 额外要求 `submit_result` 的源文件属于当前 Client 的资料目录范围。Client A 不能提交只授权给 Client B 的目录，即使该目录位于全局 `ALLOWED_SEND_ROOTS`。
