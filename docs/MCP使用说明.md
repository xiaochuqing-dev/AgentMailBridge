...
AgentMailBridge v1.3.0 MCP 是本机按需启动的通用 stdio 服务。它不监听端口、不常驻、不创建快捷方式或托盘；兼容 MCP 的本机 Agent 启动进程，stdin 关闭后服务退出。七工具、`submit_result` 参数、只读 Gmail scope 和固定 owner 安全边界保持兼容。不同客户端使用同一个服务、配置和工具，不存在客户端专用业务端点。
...
- `search_mails`：按 latest、today、yesterday、recent_days、date_range 或 all 搜索，支持 query、主题、解码后的联系人显示名/地址、收件人、有无附件、状态、排序、分页及 `ensure_fresh`。
