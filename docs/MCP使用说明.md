...
GUI 允许用户通过 Windows 文件选择器手动选择任意位置文件，但该能力没有 MCP 工具、协议参数或可伪造的 source 字段。MCP 传入桌面、下载目录或其他磁盘路径仍返回 path_not_allowed；只有明确加入 ALLOWED_SEND_ROOTS 的目录例外。
...
调试时先运行 python -m agent_mail_bridge diagnose-qq-smtp，再在 GUI Agent 接口页和日志页核对 request_id、状态和错误代码。邮箱秘密值存放于 Windows Credential Manager，MCP 不提供读取接口；不要把 .env、credentials.json 或 token.json 交给 Agent。
