...
文件仍须来自明确允许根目录。通过验证后，AgentMailBridge 自己原子复制到 `DATA_ROOT/send/staging/mcp`，再验证源文件与 staging 的字节数和 SHA-256；之后校验 SMTP 附件读取副本与 sent 归档。Agent 不应自行运行 PowerShell Copy-Item。
