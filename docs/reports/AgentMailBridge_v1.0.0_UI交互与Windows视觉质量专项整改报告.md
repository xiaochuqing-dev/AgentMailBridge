...
- Windows 字体与 DPI：集中定义 11 类 Typography Token；注册 Microsoft YaHei UI Regular/Bold，字重收敛为 400/700，Qt 6 保持 PassThrough 高 DPI 策略；旧窗口几何限制到当前屏幕可用区。收件页使用 1050 像素高内容页和常驻页面滚动条，恢复舒展留白，今日文件与最近日志各保留至少 220 像素高度并各自内部滚动；文件表格区域的滚轮直接驱动页面滚动，避免嵌套滚动导致日志无法到达。右侧面板保留独立滚动。

## 3. UI QA

实际运行源码 GUI 成功，主窗口标题正确。使用真实 Qt 渲染分别检查 100%、125%、150%、150% 紧凑窗口和深色主题，并单独检查文件操作列、收件偏好、连接健康和主题按钮。结果均通过：中文无缺字、字重稳定、无重叠；在 150%、1240×680 窗口下，中间工作区加宽，文件表格五列与操作按钮同屏可见，路径按约 30% 比例精简显示，1050 像素高内容页可顺畅滚动至宽松的最近日志区域。截图保存于 gitignored 的 artifacts/gui-qa-20260713，不进入 Git。

## 4. 自动化测试与文档

完整测试结果：295 passed in 233.83s。新增 10 项专项回归，覆盖四类收件状态、唯一刷新、精简路径与完整底层值、打开与复制、缺失文件保护、双击预览、偏好保存、五项健康状态、字体/主题图标，以及高 DPI 屏幕边界、常驻页面滚动、表格滚轮转发和日志可达性。
...
功能提交 8ee3f27、高 DPI 修复提交 710ef41、初次日志可见性修复 066575a 和最终舒展滚动布局修复 0d02c5c 均按正常提交处理，未 force push、未改写历史、未创建 GitHub Release。
...
- 安装器 SHA-256：3E3121D196DFBF2555D6055FB13978B477A417EDB0703EE49ADA59D06E2B2B16
- Portable ZIP：release/AgentMailBridge-1.0.0-Windows-x64.zip
- ZIP SHA-256：B315A08078A395B86FEEC64A556C59D5860C7C9769E522B49CFB275BC29A3A76

Windows Defender 自定义扫描未发现威胁。安装器、GUI EXE 和 MCP EXE 的 Authenticode 状态均为 NotSigned，未伪造签名。

最终安装器静默覆盖安装成功，安装前后用户数据文件数量和总字节数一致；861 个已安装文件与最终 dist 完全一致，版本均为 1.0.0。桌面快捷方式指向正式安装目录的 AgentMailBridge.exe，实际启动进程路径一致。144 DPI 下窗口为 1240×680，中间工作区、精简路径、同屏操作按钮、常驻页面滚动和宽松日志表格均为最终界面；第二次启动仍只有一个进程。
