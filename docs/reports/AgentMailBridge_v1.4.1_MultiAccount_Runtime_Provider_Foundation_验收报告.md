...
实现提交为 `ebf474c`（`feat: activate multi-account runtime foundation`）和 `6ea0164`（`feat: expose multi-account controls in GUI and MCP`）；验收文档提交为 `06cf1dc`（`docs: document v1.4.1 runtime verification`）。三笔提交已从基线 `97be878` 快进推送到 `origin/master`；本最终状态补记继续使用普通 push。没有 force push、删除历史、Tag、GitHub Release 或公开 Release Assets。
