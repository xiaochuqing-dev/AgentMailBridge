...
v1.6.0 正式重点支持 Codex、Claude Code 和 Hermes。普通 Claude Desktop 只保留已有兼容代码，不在推荐入口和真实验收范围。AgentMailBridge 仍只提供七个确定性事实工具，不内置模型、不理解自然语言、不自动挑选邮件，也不建立通用 IAM、团队角色或远程认证系统。
