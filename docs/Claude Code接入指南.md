...
v1.7.1 正式支持 Claude Code。先在 GUI 的“Agent 接入”连接 Claude Code，再明确设置读取账号/目录、发件账号、确认或自主发送和本地附件目录。普通 Claude Desktop 是另一个入口，只保留兼容配置。Claude Code 可查询恢复状态，但不能确认或自动重发 `delivery_unknown`。
