...
v1.7.2 把 Codex Desktop 与 Codex CLI 作为推荐入口。先在 GUI 的“Agent 接入”连接 Codex，再用人话分别授权读取账号/目录、发件账号、确认或自主发送和本地附件目录。桌面真实任务必须实际调用工具，不能用 CLI 或配置存在替代。结果不确定发件只能查询、对账或由 GUI 用户处理，Agent 不得自动重发。
...
v1.7.2 已真实验证 Codex Desktop 独立新任务和 Codex CLI 0.146.0。Desktop 验收覆盖账号/目录发现、Inbox/Sent 搜索、邮件与资源读取、完整资料准备，以及暂停后下一次调用立即拒绝；CLI 验收使用最终安装版 MCP。0.145.0 继续兼容，其他版本仍按未知版本处理，不自动写入 TOML。
