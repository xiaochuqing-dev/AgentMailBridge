...
确认时重新解析 Client token，检查 active/paused/revoked、`mail.send`、发件账号、源邮件读取范围、附件目录、到期时间、文件大小和 SHA-256。同一 Client 与 idempotency key 只执行一次；重复确认返回既有结果。

真实执行先取得跨进程 lease，并固定 Message-ID、MIME bytes 和 SHA-256。SMTP DATA 前中断恢复为确定未发送；DATA 后缺少接受证据进入 `delivery_unknown`，绝不会自动重发；SMTP 已接受但本地归档失败进入 `sent_archive_failed`，只恢复原 MIME 的本地事实。详见《发件状态机与崩溃恢复》。
