...
- 收件：标题右侧唯一刷新、自动收取、检查间隔、连接测试、当前增量“立即收取”、可取消“历史补扫”、可编辑收件偏好、今日文件和最近日志。Gmail 管理只保留在左侧账号卡。
- 发件：To 默认填入 `OWNER_GMAIL` 且可由用户改成一个明确合法地址；编辑可选主题、正文、0 至多个附件和 0 至多个链接，一次确认发送一封邮件。附件和链接表格使用整行样式，最近发送与详情保存并显示真实 To。
