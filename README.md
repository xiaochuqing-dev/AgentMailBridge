...
AgentMailBridge v1.4.3 是面向个人用户的本地优先 Windows 邮箱桥接工具。Multi-Account Runtime 让稳定 `account_id` 驱动账号配置、凭据、OAuth、收发、同步状态、重试和退避。本版继续硬化共享 Generic IMAP/SMTP Core，新增 UIDVALIDITY 代际化重试、国际化目录解码、协议错误脱敏、SMTP 临时/永久失败分类、受控 Provider 验收脚本和完整测试前 Preflight。兼容本地 stdio MCP 的受控邮件读取、资源准备和固定目标结果回邮保持不变。
...
运行 `AgentMailBridge-1.4.3-Setup.exe`。默认安装到 `%LOCALAPPDATA%\Programs\AgentMailBridge`，无需 Python、Git 或管理员权限。桌面和开始菜单只指向 `AgentMailBridge.exe`；内部 `AgentMailBridgeMCP.exe` 不创建快捷方式、托盘或开机启动项。
...
第一次完整 pytest 前应先运行 `python scripts\full_suite_preflight.py`。它检查版本、Provider 状态、schema、已知硬编码断言、`git diff --check`、compileall，并执行 v1.4.3、Generic、多账号、GUI 与包装定向回归。真实 Provider 验收使用已配置账号的 Credential Manager 凭据；脚本不接受命令行密码，无 `--confirm-network` 不联网，无 `--confirm-real-send` 不发信：
...
- `release\AgentMailBridge-1.4.3-Setup.exe`
- `release\AgentMailBridge-1.4.3-Windows-x64.zip`
