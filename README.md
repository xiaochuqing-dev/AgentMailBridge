...
2. 在首次向导填写 Gmail、数据目录和可选的 QQ 发件信息。
3. Gmail IMAP 应用专用密码和 QQ SMTP 授权码保存到 Windows Credential Manager，不写入普通配置。
4. 使用 Gmail API 时，在“高级设置”点击“导入 OAuth JSON”，选择从 Google Cloud Console 下载的 Desktop OAuth `credentials.json`，再点击“Gmail API 显式授权”。
...
GUI 的“Agent 接口”页面可复制 Codex、Claude Code 和通用 JSON 配置。安装版通用结构如下，实际绝对路径由 GUI 自动生成：
...
当前自动化测试为 251 项。packaged MCP 已验证 initialize、notifications/initialized、ping、tools/list、submit_result、fixed recipient、path_not_allowed、request_id、duplicate、审计、stdio 纯净和 EOF 自动退出；真实 QQ SMTP 发送已验证 SMTP 接受、本地 sent 归档及 SHA-256 一致，但未宣称 Gmail 最终收到。
...
- 当前机器完成了安装版 Credential Manager、Gmail API 和 QQ SMTP 诊断，但尚缺真正独立无 Python Windows 环境、跨版本旧候选升级和完整 DPI/托盘人工视觉矩阵。
