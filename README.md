...
AgentMailBridge v1.4.2 是面向个人用户的本地优先 Windows 邮箱桥接工具。Multi-Account Runtime 让稳定 `account_id` 驱动账号配置、凭据、OAuth、收发、同步状态、重试和退避。本版在既有 Gmail 收件、统一归档和 QQ 发件链路上接通共享 Generic IMAP/SMTP Core，QQ、163 与手工配置的 Generic 账号可复用同一套收件、发件、归档、历史补扫和调度能力。兼容本地 stdio MCP 的受控邮件读取、资源准备和固定目标结果回邮保持不变。
...
| QQ | IMAP | SMTP | QQ 邮箱授权码 | 实现与自动化通过；真实账号 E2E NOT_TESTED |
| 163 | IMAP | SMTP | 163 邮箱授权码 | 实现与自动化通过；真实账号 E2E NOT_TESTED |
| Generic | 按配置启用 IMAP | 按配置启用 SMTP | 账号级 IMAP/SMTP secret | implementation ready；实际服务器必须连接测试 |
...
运行 `AgentMailBridge-1.4.2-Setup.exe`。默认安装到 `%LOCALAPPDATA%\Programs\AgentMailBridge`，无需 Python、Git 或管理员权限。桌面和开始菜单只指向 `AgentMailBridge.exe`；内部 `AgentMailBridgeMCP.exe` 不创建快捷方式、托盘或开机启动项。
...
检查间隔决定“多久检查一次”，Gmail lookback 或 IMAP UID overlap 决定“每次增量向前重查多少”，两者不是同一概念。改变检查间隔不会扩大历史范围；更早邮件应使用历史补扫。历史补扫直接查询指定日期范围，按账号、Message-ID 与 provider id 去重，不删除邮件、不执行附件；Gmail API 保持只读，IMAP 使用 `BODY.PEEK[]` 不误标已读。UIDVALIDITY 变化会安全重置该 mailbox 的 UID 游标，再依靠归档去重重扫，避免漏信。
...
- `release\AgentMailBridge-1.4.2-Setup.exe`
- `release\AgentMailBridge-1.4.2-Windows-x64.zip`
