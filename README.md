...
AgentMailBridge v1.3.0 是面向个人用户的本地优先 Windows 邮箱桥接工具。它在后台通过 Gmail 收取并归档完整邮件，通过 QQ SMTP 发送用户明确编写的邮件，并向兼容本地 stdio MCP 的 Agent 提供受控邮件读取、资源准备和固定目标结果回邮能力。GUI 不是 Agent 读取本地邮件归档的必经路径。
...
运行 `AgentMailBridge-1.3.0-Setup.exe`。默认安装到 `%LOCALAPPDATA%\Programs\AgentMailBridge`，无需 Python、Git 或管理员权限。桌面和开始菜单只指向 `AgentMailBridge.exe`；内部 `AgentMailBridgeMCP.exe` 不创建快捷方式、托盘或开机启动项。
...
- `release\AgentMailBridge-1.3.0-Setup.exe`
- `release\AgentMailBridge-1.3.0-Windows-x64.zip`
