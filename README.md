...
AgentMailBridge v1.4.0 是面向个人用户的本地优先 Windows 邮箱桥接工具。本阶段进入 Multi-Account Core：邮箱账号成为一等实体，邮件、邮箱目录、同步、重试、归档和发件事实都能追溯到稳定账号 ID。现有可用能力仍是 Gmail API/IMAP 收件与归档、QQ SMTP 发件，以及兼容本地 stdio MCP 的受控邮件读取、资源准备和固定目标结果回邮；GUI 不是 Agent 读取本地归档的必经路径。
...
`MailAccount` 使用 provider 与规范化邮箱地址生成稳定、不含明文地址的 `account_id`，并单独声明认证类型、收件/发件开关、能力和数据命名空间。Provider Adapter 只复用既有协议实现：Gmail 当前接通收件、归档和 Mail Facts，QQ 当前接通发件与发送归档；Gmail 发件、QQ 收件、163、Outlook/Microsoft 和 Generic IMAP/SMTP 只是扩展边界，尚未正式支持，也未在 GUI 开放新增第二个同类型账号。

SQLite 新增 `mail_accounts`、`mailboxes` 和 `account_sync_states`，并给邮件 package、兼容收件行、重试、规则评估、outbound 与 sent 事实补充账号归属。v1.3.0 旧 Gmail/QQ 配置在首次启动时先备份数据库，再于同一可回滚事务中映射为正式账号；迁移不移动归档、不改写 `raw.eml` 或历史 Hash，且可重复执行。底层账号独立，上层 Mail Facts 与统一 MCP 默认仍可跨账号查询；`search_mails` 可用 `account_id` 精确限定。
...
- 顶部主工作区只有“收件”和“发件”。
- 左侧统一“邮箱账号”列表展示 Gmail、QQ 账号卡片和当前能力，并预留可滚动的多账号结构；“添加邮箱账号”仅展示未来扩展说明，不新增第二个同类型账号。
...
- 发件页支持一个用户明确输入的合法 To、可选主题、正文、0 至多个附件和 0 至多个链接；发送前展示实际收件人并拒绝空值、非法地址、多地址和 CRLF 注入。一次确认只生成并发送一封 MIME 邮件，最近发送与历史记录保存真实 To。
...
运行 `AgentMailBridge-1.4.0-Setup.exe`。默认安装到 `%LOCALAPPDATA%\Programs\AgentMailBridge`，无需 Python、Git 或管理员权限。桌面和开始菜单只指向 `AgentMailBridge.exe`；内部 `AgentMailBridgeMCP.exe` 不创建快捷方式、托盘或开机启动项。
...
检查间隔决定“多久检查一次”，重叠 lookback 决定“每次增量向前回看多久”，两者不是同一概念。改变检查间隔不会扩大历史范围；离开增量窗口的旧邮件应使用历史补扫。历史补扫直接查询指定日期范围，按 Message-ID 与 provider id 去重，不删除邮件、不执行附件；Gmail API 保持只读，IMAP 使用 `BODY.PEEK[]` 不误标已读。

调度状态按账号持久化保存上次检查、上次成功、最近结果、下次检查、连续全局失败和 checkpoint，并保留 v1.3 全局状态兼容快照。认证、网络或后端整体失败按 30 秒、1 分钟、2 分钟、5 分钟、最长 15 分钟退避，成功后恢复正常周期；睡眠或长暂停后由超时看门狗立即补偿。窗口进入托盘时调度继续，只有真正退出才停止。
...
- `release\AgentMailBridge-1.4.0-Setup.exe`
- `release\AgentMailBridge-1.4.0-Windows-x64.zip`
