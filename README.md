...
AgentMailBridge v1.4.1 是面向个人用户的本地优先 Windows 邮箱桥接工具。Multi-Account Runtime 让稳定 `account_id` 真正驱动账号配置、凭据、OAuth、收件、GUI 手工发件、同步状态、重试和退避。正式可用能力仍是 Gmail API/IMAP 收件与归档、QQ SMTP 发件；Generic IMAP/SMTP 已建立实验性连接测试、目录发现、SPECIAL-USE 与 UID checkpoint 基础，但尚未开放正式收发。兼容本地 stdio MCP 的受控邮件读取、资源准备和固定目标结果回邮保持不变。
...
- Gmail：receive、archive、Mail Facts 正式支持；send 未实现。官方 Gmail API 发件需要 `gmail.send` 或 `gmail.compose`，与产品强制且唯一的 `gmail.readonly` 安全边界冲突，因此 v1.4.1 不扩大 scope、不触发用户重新授权。
- QQ：send 与 outbound archive 正式支持；receive 未实现。
- Generic IMAP/SMTP：Provider Profile、TLS 配置校验、连接测试、目录发现和 checkpoint 为实验性基础；receive/send 未实现。
- 163、Outlook/Microsoft 及其他 Provider：planned，未宣称可用。
...
- 顶部主工作区只有“收件”和“发件”。
- 左侧统一“邮箱账号”列表展示真实账号、Provider、地址、能力和启停状态；“添加邮箱账号”可创建 Gmail、QQ 与 Generic 基础账号，动态账号页可测试连接、发现目录、管理按账号凭据/OAuth，并安全移除账号。
...
运行 `AgentMailBridge-1.4.1-Setup.exe`。默认安装到 `%LOCALAPPDATA%\Programs\AgentMailBridge`，无需 Python、Git 或管理员权限。桌面和开始菜单只指向 `AgentMailBridge.exe`；内部 `AgentMailBridgeMCP.exe` 不创建快捷方式、托盘或开机启动项。
...
- `release\AgentMailBridge-1.4.1-Setup.exe`
- `release\AgentMailBridge-1.4.1-Windows-x64.zip`
