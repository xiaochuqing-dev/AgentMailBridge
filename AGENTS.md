...
- AgentMailBridge v1.7.2 is the current product version. It is a reliability audit and release-candidate closure that prevents Message-ID-only Sent reconciliation or local send recovery while preserving the v1.7.1 fact, permission, provider and lifecycle boundaries.
...
For v1.7.2, develop directly on `master`. Do not create a branch, force-push, rewrite history, create a Tag or publish a GitHub Release. Commit the completed implementation, tests, migrations, GUI, documentation and final report normally, then push to `origin/master`.
